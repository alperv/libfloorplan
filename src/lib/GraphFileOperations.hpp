...
string GetAttr(const ptree::value_type v, std::string attrName)
{
...
string formatStringForDot(string str){
    if (str.size() != 0){
        if (isdigit(str[0]))
            str = "S" + str;
...
/**
  ...
  */
graphProperties loadGraphFromXML(std::string filenamePath, floorplanGraph& outGraph, std::string rootNodeName = "MITquest"){
    ptree XMLtree;
    ...
    BOOST_FOREACH(ptree::value_type &v, XMLtree.get_child(rootNodeName.c_str()))
    {
        if( ((string)v.first.data()).compare("space") == 0 )
        {
            ...
            if (!spaceName.empty()){
                pair<bool,Vertex> searchResult =  doesVertexExists(spaceName, outGraph);
...
/**
  ...
  */
std::vector<graphProperties> loadAllGraphsInFolder(string sDir, vector<floorplanGraph>& vResult, int iLimit = -1, std::string rootNodeName  = "MITquest")
{
    DIR *dp;
    stringstream fLoad;
    struct dirent *dirp;
...
/**
  ...
  */
void saveGraphToXML(std::string filenamePath, const floorplanGraph& Graph){
...
void saveGraphToDot(std::string filenamePath, const floorplanGraph& Graph){
...
/**
  ...
  */
void saveGraphToPNG(std::string filenamePath, const floorplanGraph& Graph){
    saveGraphToDot(filenamePath,Graph);
    system(("neato -Tpng -Gcharset=latin1 " + filenamePath + " > " + filenamePath + ".png").c_str());
}


void saveGraphLayoutToPNG(std::string filenamePath, const floorplanGraph& Graph){
